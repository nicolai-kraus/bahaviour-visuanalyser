...
def interaction_network(df, threshold=1):
    ...
    G_dot.format= 'svg'
    G_dot.render('uploads/interactions.gv', view=False)  
    location = '../uploads/interactions.gv.svg'
...
def dataplot(df, behavior, show_grid):
    """
    ...
    fig.savefig('uploads/data_plot.png', bbox_inches='tight')
    location = '../uploads/data_plot.png'

    return location


def transition_network(df):
    ...
    G_dot = graphviz.Source(G_dot_string)
    G_dot.format= 'svg'
    G_dot.render('uploads/transitions.gv', view=False)  
    location = '../uploads/transitions.gv.svg'
